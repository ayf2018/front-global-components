...
<div class="left-nav-main">
  <header-drawer
    ...
    v-on:return-current-item="returnCurrentItem"
  ></header-drawer>
  <div class="leftNav">
    <div :style="{'height':navHeight+'px'}" class="leftBox">
      <ul :class="{'showPage':total>page.size}">
        ...
        >
          ...
          <img v-if="t.state" class="state-img" :src="currentIndex===i?t.stateActiveicon:t.stateicon">
        </li>
        <li
          class="errLi"
          v-if="renderList.length===0 && !isLoading"
        ...
        <li
          class="loadings-spin-container"
          v-if="isLoading"
        >
          <Spin fix class="mySpin">
            </Spin>
        </li>
      </ul>
      <my-page
        v-show="total>page.size"
    ...
    <div v-if="currentType==='style2'" class="query">
        <div class="external">
            <Input v-model="queryVal" placeholder="输入船只名称" />
            <a @click="query"></a>
        </div>
    </div>
    <div v-if="currentType==='style2'" class="message">
...
export default {
  components: { headerDrawer, myPage },
  props: {
    bottomHeight: {
      type: Number,
      default: 0
  ...
  methods: {
    ...
    initPage () {
      if (this.navHeight / 45 > this.list.length) { // 一页放不下
  ...
  computed: {
    ...
    renderList () {
      ...
      let temp = this.list.slice((this.page.current - 1) * this.page.size, this.page.current * this.page.size)
